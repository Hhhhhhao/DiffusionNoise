...
from diffusers.pipelines.latent_diffusion.pipeline_latent_diffusion import ClassEmbedder, LDMTextToImagePipeline
...
def load_ldm_model(model_name):
    pipeline = LDMTextToImagePipeline.from_pretrained(model_name)
...
def main():
    ...
    parser.add_argument(
        "--model_list",
        nargs="+",
        default=['DiffusionNoise/ldm_cc3m_clean', 'DiffusionNoise/ldm_cc3m_random_noise_2.5', 'DiffusionNoise/ldm_cc3m_random_noise_5',  'DiffusionNoise/ldm_cc3m_random_noise_10'],
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    # split classes
    parser.add_argument(
        "--prompt", type=str, help="class synset", default=0, nargs='+',
    )
    ...
    for prompt_idx, prompt in tqdm(enumerate(prompt_list)):
        
        # generate a uniqe number
        folder_name = str(np.random.randint(0, 100000))
        ...
        for i in tqdm(range(n_samples_per_class)):
            ...
            model_generator = torch.Generator(device='cuda').manual_seed(int(opt.seed) + int(prompt_idx))
            ...
            for model in model_list:
                ...
                for j in range(0, len(latents), batch_size):
                    ...
                    if 'ldm' in model:
                        images = pipeline([prompt] * batch_size, num_inference_steps=opt.ddim_steps, eta=opt.ddim_eta, guidance_scale=opt.scale, latents=latents[j:j+batch_size], generator=model_generator, output_type='np').images
                    elif 'dit' in model:
                ...
                save_path_list = [os.path.join(sample_path, f"{model_name}/{i:03d}.jpeg") for i in range(len(model_images))]
            ...
            save_path = os.path.join(sample_path, f"grid.jpeg")
